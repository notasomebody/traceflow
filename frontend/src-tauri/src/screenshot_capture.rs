...
impl Default for ScreenshotSettings {
    fn default() -> Self {
        Self { enabled: false, interval_minutes: 10, retain_raw_days: 0 }
    }
}
...
impl ScreenshotRuntime {
    pub fn new(data_dir: &Path) -> Self {
        ...
        let settings = std::fs::read_to_string(&settings_path).ok()
            ...
            .unwrap_or_default();
        Self { settings: Arc::new(Mutex::new(settings)), settings_path, screenshot_dir: data_dir.join("screenshots") }
    }

    pub fn settings(&self) -> Result<ScreenshotSettings, String> {
        self.settings.lock().map(|value| value.clone()).map_err(|_| "截图设置不可用".into())
    }

    pub fn save_settings(&self, settings: ScreenshotSettings) -> Result<ScreenshotSettings, String> {
        let settings = settings.validate()?;
        if let Some(parent) = self.settings_path.parent() {
            std::fs::create_dir_all(parent).map_err(|error| error.to_string())?;
        }
        std::fs::write(&self.settings_path, serde_json::to_vec(&settings).map_err(|error| error.to_string())?)
            .map_err(|error| error.to_string())?;
        *self.settings.lock().map_err(|_| "截图设置不可用".to_string())? = settings.clone();
        Ok(settings)
    }

    pub fn start(&self, monitor: MonitorRuntime) {
        let runtime = self.clone();
        let _ = std::thread::Builder::new().name("traceflow-screenshot-ocr".into()).spawn(move || {
            let mut last_capture = 0u64;
            loop {
                let settings = runtime.settings().unwrap_or_default();
                let now = unix_seconds();
                let due = now.saturating_sub(last_capture) >= settings.interval_minutes as u64 * 60;
                if settings.enabled && due {
                    if let Some(window) = active_window().filter(|window| monitor.can_capture_now(&window.application_name)) {
                        let _ = runtime.capture_ocr_and_store(&window.application_name, &settings);
                        last_capture = now;
                    }
                }
    ...
    pub fn capture_preview(&self) -> Result<ScreenshotPreview, String> {
        ...
        let file = self.screenshot_dir.join(format!("preview-{}.bmp", unix_millis()));
    ...
    fn capture_ocr_and_store(&self, application_name: &str, settings: &ScreenshotSettings) -> Result<(), String> {
        std::fs::create_dir_all(&self.screenshot_dir).map_err(|error| error.to_string())?;
        ...
        if settings.retain_raw_days == 0 {
            ...
            let protected = protect_bytes(&std::fs::read(&raw).map_err(|error| error.to_string())?)?;
    ...
    fn cleanup_encrypted_raw(&self, retain_days: u8) -> Result<(), String> {
        if !self.screenshot_dir.exists() { return Ok(()); }
        ...
        for entry in std::fs::read_dir(&self.screenshot_dir).map_err(|error| error.to_string())? {
            ...
            if entry.path().extension().and_then(|value| value.to_str()) != Some("tfshot") { continue; }
            let age = entry.metadata().and_then(|metadata| metadata.modified())
                .ok().and_then(|modified| modified.elapsed().ok()).unwrap_or_default();
            if retain_days == 0 || age > maximum_age { let _ = std::fs::remove_file(entry.path()); }
        }
        Ok(())
    }
}
...
fn send_ocr_observation(application_name: &str, recognized_text: &str) -> Result<(), String> {
    ...
    if response.starts_with("HTTP/1.1 201") || response.starts_with("HTTP/1.0 201") { Ok(()) } else { Err("本地 OCR 文本入库失败".into()) }
}
...
fn protect_bytes(plaintext: &[u8]) -> Result<Vec<u8>, String> {
    ...
    unsafe {
        ...
        CryptProtectData(&input, w!("TraceFlow encrypted screenshot"), None, None, None, CRYPTPROTECT_UI_FORBIDDEN, &mut output)
            .map_err(|error| error.to_string())?;
...
fn local_ocr(path: &Path) -> Result<String, String> {
    ...
    let winrt_path = absolute_text.strip_prefix(r"\\?\").unwrap_or(absolute_text.as_ref());
    let file = StorageFile::GetFileFromPathAsync(&HSTRING::from(winrt_path))
        .map_err(|error| error.to_string())?.get().map_err(|error| error.to_string())?;
    let stream = file.OpenAsync(FileAccessMode::Read).map_err(|error| error.to_string())?
        .get().map_err(|error| error.to_string())?;
    let decoder = BitmapDecoder::CreateAsync(&stream).map_err(|error| error.to_string())?
        .get().map_err(|error| error.to_string())?;
    let bitmap = decoder.GetSoftwareBitmapAsync().map_err(|error| error.to_string())?
        .get().map_err(|error| error.to_string())?;
    let engine = OcrEngine::TryCreateFromUserProfileLanguages().map_err(|error| error.to_string())?;
    let result = engine.RecognizeAsync(&bitmap).map_err(|error| error.to_string())?
        .get().map_err(|error| error.to_string())?;
...
fn capture_active_window(path: &Path) -> Result<(), String> {
    ...
    unsafe {
        ...
        if window.0.is_null() { return Err("未找到活动窗口".into()); }
        let mut rect = RECT::default();
        GetWindowRect(window, &mut rect).map_err(|error| error.to_string())?;
        let width = rect.right - rect.left;
        let height = rect.bottom - rect.top;
        if width <= 0 || height <= 0 { return Err("活动窗口尺寸无效".into()); }

        let source_dc = GetWindowDC(Some(window));
        if source_dc.0.is_null() { return Err("无法读取活动窗口画面".into()); }
        ...
        let result = BitBlt(memory_dc, 0, 0, width, height, Some(source_dc), 0, 0, SRCCOPY);
        ...
        let scan_lines = GetDIBits(memory_dc, bitmap, 0, height as u32, Some(pixels.as_mut_ptr().cast()), &mut info, DIB_RGB_COLORS);
        SelectObject(memory_dc, old);
        let _ = DeleteObject(HGDIOBJ(bitmap.0));
        let _ = DeleteDC(memory_dc);
        let _ = ReleaseDC(Some(window), source_dc);
        if scan_lines == 0 { return Err("无法读取截图像素".into()); }
...
mod tests {
    ...
    fn raw_retention_uses_only_explicit_supported_windows() {
        for days in [0, 1, 3, 7] {
            let settings = ScreenshotSettings { retain_raw_days: days, ..Default::default() };
    ...
    fn windows_ocr_recognizes_a_controlled_test_image_when_provided() {
        let Some(path) = std::env::var_os("TRACEFLOW_OCR_TEST_IMAGE") else { return; };
        let text = super::local_ocr(std::path::Path::new(&path)).expect("Windows OCR should read the controlled image");
        let normalized = text.to_uppercase().replace(' ', "");
        assert!(normalized.contains("TRACEFLOW"), "recognized text was: {text}");
    }
}
