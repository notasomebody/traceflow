...
export default function SettingsPanel({ settings, supportEmail, onChange, onClose, onRestartGuide }: Props) {
  ...
  const makeCode = async () => { const code = await exportShareCode(settings, password); setShareCode(code); setMessage(password ? "已生成密码加密分享码" : "已生成离线分享码"); };
  const readCode = async () => { try { const imported = await importShareCode(shareCode, password); setPreview(imported); setMessage("校验通过，请确认差异后应用"); } catch (error) { setPreview(undefined); setMessage(error instanceof Error ? error.message : "导入失败"); } };
  ...
  return <div className="drawer-backdrop" onMouseDown={onClose}><section className="settings-drawer" onMouseDown={event => event.stopPropagation()}>
    ...
    <div className="settings-body">
      {tab === "general" && <><h3>工作偏好</h3><div className="settings-fields"><label>每日默认工时<input type="number" min="0" max="24" step="0.5" value={settings.targetMinutes / 60} onChange={event => update("targetMinutes", Number(event.target.value) * 60)}/></label><label>日报生成时间<input type="time" value={settings.generateAt} onChange={event => update("generateAt", event.target.value)}/></label><label>允许提交时间<input type="time" value={settings.submitAfter} onChange={event => update("submitAfter", event.target.value)}/></label></div><label className="toggle-row"><span><strong>关闭窗口时缩小到托盘</strong><small>保证定时生成和数据采集继续运行</small></span><input type="checkbox" checked={settings.minimizeToTray} onChange={event => update("minimizeToTray", event.target.checked)}/></label><button className="secondary-action" onClick={onRestartGuide}><RotateCcw/>重新运行首次引导</button></>}
      {tab === "privacy" && <><h3>本机数据保护</h3><div className="privacy-cards"><article><LockKeyhole/><div><strong>账号凭据</strong><p>密码和令牌只进入 Windows 凭据管理器或 macOS 钥匙串，不写入 SQLite 和日志。</p></div></article><article><ShieldCheck/><div><strong>工作内容</strong><p>敏感正文使用 AES-256-GCM 加密；AI 发送前展示内容并支持脱敏。</p></div></article></div><label className="toggle-row"><span><strong>默认仅采集元数据</strong><small>正文需要在每个数据源中主动授权</small></span><input type="checkbox" checked={settings.metadataOnly} onChange={event => update("metadataOnly", event.target.checked)}/></label><label className="toggle-row"><span><strong>仅本机使用统计</strong><small>记录使用次数、成功率和耗时，不上传、不记录正文</small></span><input type="checkbox" checked={settings.localStatistics} onChange={event => update("localStatistics", event.target.checked)}/></label></>}
