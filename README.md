...
- 从“历史记录”导入旧日报，并在本机加密保存
- 经授权后用 UI Automation 读取当前打开的企业微信汇报窗口，再由用户检查导入
...
左侧导航采用独立页面切换：无论当前位于“设置”“历史记录”还是“企业微信”，都可以直接点击“今日工作”返回，不需要滚动页面。

## 历史日报与企业微信

- 点击左侧“历史记录”→“导入历史日报”，填写日期、工作总结和下一步计划；确认后正文加密保存到本机数据库。
- 点击左侧“企业微信”可粘贴历史汇报，也可在 Windows 桌面版中读取当前打开的企业微信窗口。
- 自动读取前，需在“设置 → 数据与隐私”单独开启 UI Automation。它只读取当前活动窗口，不会遍历全部聊天或汇报历史，也不会触发截图 OCR。
